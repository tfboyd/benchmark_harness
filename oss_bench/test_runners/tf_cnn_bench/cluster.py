...
class AWSInstance(object):
  ...
  def WaitUntilReady(self):
    ...
    # hostname is used to create services and public_host is used to connect
    # to the machine.
    self.hostname = self.aws_instance.private_ip_address
    self.public_host = self.aws_instance.public_dns_name

  def CreateSshClient(self):
    assert self.public_host is not None
    ssh_client = util.SshToHost(self.public_host, ssh_key=self.ssh_key, username=self.username)
  ...
  def reuse_ssh_client(self):
    ...
    if not hasattr(self, 'ssh_client') or self.ssh_client == None:
      self.ssh_client = util.SshToHost(self.public_host, ssh_key=self.ssh_key, username=self.username)
...
def AwsInstances(num_instances=1,
  ...
  try:
    ...
    print('Exception setting up AWS instance:{}'.format(e))
