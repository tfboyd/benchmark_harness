...
class TestTracker(unittest.TestCase):
  ...
  def test_check_state_false(self, mock_get_state_object):
    ...
    found = tracker.check_state(None, 'tensorflow', 'RC', 'OTB.GPU',
                                '1.4_02102018', 'tf_cnn_bench')
  ...
  def test_check_state_true(self, mock_get_state_object):
    ...
    mock_get_state_object.return_value = self._mock_state_object()
    found = tracker.check_state(None, 'mxnet', 'FINAL', 'OTB.GPU', '1_0_0',
                                'mxnet')
    self.assertTrue(found)

  @patch('tools.tracker._save_state_object')
  @patch('tools.tracker._get_state_object')
  def test_update_state_round_trip(self, mock_get_state_object,
    ...
    self.assertFalse(found)
    tracker.update_state(None, 'tensorflow', 'RC', 'OTB.GPU', '1.4_02102018',
                         'tf_models')
    saved_object = mock_save_state_object.call_args[0][1]
    mock_get_state_object.return_value = saved_object
    found_post_update = tracker.check_state(None, 'tensorflow', 'RC', 'OTB.GPU',
                                            '1.4_02102018', 'tf_models')
  ...
  def test_update_state_new_key(self, mock_get_state_object,
    ...
    mock_get_state_object.return_value = self._mock_state_object()
    tracker.update_state(None, 'tensorflow', 'FINAL', 'OTB.GPU', '1.4_02102018',
                         'tf_models')
    saved_object = mock_save_state_object.call_args[0][1]
